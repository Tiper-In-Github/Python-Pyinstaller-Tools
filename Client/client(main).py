...
class LoginFrame(wx.Frame):
    ...
    def Version_checking(self):  # 版本检查
        ...
        try:
            ...
            ip_port = ("106.12.103.112", 6979)  # 地址和端口号 填localhost可以进行本地测试
    ...
    def login(self, e):
        ...
        if userinname == "['FPA']":
            ...
            servername = 'FPA'
            try:
                con.open('182.61.28.124', port='6666', timeout=10)  # 正式连接服务器
            except:
                winsound.PlaySound("s01.wav", winsound.SND_ASYNC)
            ...
            servername = '樱花城'
            try:
                con.open('182.61.28.124', port='5555', timeout=10)  # 正式连接服务器
            except:
                winsound.PlaySound("s01.wav", winsound.SND_ASYNC)
                dlg = wx.MessageDialog(self, "连接失败\n请检查你是否输入了正确的索引（服务器名）\n或网络是否正常，如果以上情况均不存在.\n可能服务器已经不再支持当前版本，请到官网更新软件\n - www.huoyinetwork.cn", "连接失败", wx.OK)
                dlg.ShowModal()
